...
function MarkdownMessage({ text, isUser }: { text: string; isUser: boolean }) {
  const codeBlockStyle = "bg-ink-100 text-green-300 rounded-xl p-3 text-[11px] overflow-x-auto my-2 font-mono leading-relaxed";
  const accentColor = isUser ? 'text-white/70' : 'text-moss-500';
...
const FloatingChat: React.FC<Props> = ({ userGrade, language, context, translations, activeView, currentSession }) => {
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="fixed bottom-6 right-6 w-14 h-14 bg-gradient-to-br from-moss-500 to-moss-700 text-white rounded-2xl shadow-2xl shadow-moss-500/30 flex items-center justify-center transition-all duration-300 hover:scale-110 hover:shadow-glow z-50 rtl:left-6 rtl:right-auto animate-bounce-subtle"
      >
  ...
  return (
    ...
    >
      {/* ── Header ── */}
      <div className="flex items-center justify-between px-5 py-4 bg-gradient-to-r from-moss-600 to-moss-700 text-white flex-shrink-0">
      ...
      <div className="p-3 bg-white dark:bg-ink-100 border-t border-ink-100 dark:border-ink-200 flex-shrink-0">
        ...
        <div className="flex items-end gap-2 bg-cream-50 dark:bg-ink-100 border border-ink-100 dark:border-ink-200 rounded-2xl px-3 py-2">
          ...
          <button
            ...
            className="p-2 bg-moss-600 text-white rounded-xl hover:bg-moss-700 disabled:opacity-40 disabled:cursor-not-allowed transition-all flex-shrink-0 active:scale-95"
          >
