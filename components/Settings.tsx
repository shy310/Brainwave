...
const Settings: React.FC<Props> = ({
  ...
  onProfileUpdate, onGradeChange, onThemeToggle, onLanguageChange, onToggleSound, onToggleCelebrations
}) => {
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-8 view-enter overflow-y-auto h-full scrollbar-hide pb-20">
      ...
      <section className="bg-white dark:bg-ink-800 rounded-2xl border border-ink-100 dark:border-ink-700 p-6 shadow-sm mb-4 space-y-5">
        ...
        <label className="flex items-center justify-between mt-5 text-sm font-bold text-ink-500 dark:text-ink-400">
          {studyCopy(language).celebrations}
          <input type="checkbox" checked={user.celebrationsEnabled !== false} onChange={onToggleCelebrations} className="w-5 h-5" />
        </label>
      </section>
